...
if __name__ == '__main__':
    ...
    ###IGCROI_GANet_Ablation experiments
    '''
    vit_base = VisionTransformer(img_size=224, patch_size=16, in_chans=3, num_classes=3, embed_dim=768,
                                 depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0.,
                                 attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm)
    ...
    igcrga_net = IGCROI_GANet_Ablation(vit_base=vit_base, dense_base=dense_base)
    print(summary(igcrga_net, torch.randn((1, 3, 224, 224))))
    print(igcrga_net)
    igcrga_net = igcrga_net.cuda(gpu_device)
    '''

    ###IGCROI_GANet experiments
    ...
    ddai_net = IGCROI_GANet(vit_base=vit_base, dense_base=dense_base, work_mode=work_mode, model_mode=model_mode,
                          class_num = class_num)
    print(summary(igcrga_net, torch.randn((2, 3, 224, 224))))
    print(igcrga_net)
    igcrga_net = igcrga_net.cuda(gpu_device)
    ...
    Triplet_out_fit(ddai_net=igcrga_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
                  epoch=10, gpu_device=gpu_device, train_mode=train_mode)

    #save_model(model=igcrga_net, file_name='ViT-visual.pt', file_path='E:\DDAI-TCNet-Github\Weights\\Tian.pth')
    ...
    #model_weights = torch.load(r'E:\DDAI-TCNet-Github\Weights\breast_exp\ViT.pth',
                               #map_location='cuda:0')
    #ddai_net.load_state_dict(model_weights, strict=False)

    acc_scores(model=igcrga_net, gpu_device=gpu_device, data_loader=test_loader,
    ...
    attention_rollout = VITAttentionRollout(igcrga_net)
